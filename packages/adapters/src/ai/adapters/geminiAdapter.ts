...
async function checkGeminiAvailable(): Promise<boolean> {
  try {
    const result = await runShellCommand({
      ...
      cwd: process.cwd(),
      timeoutSeconds: 5,
      stdoutPath: join(tmpdir(), `aiteam-gemini-check-${String(Date.now())}.out`),
      stderrPath: join(tmpdir(), `aiteam-gemini-check-${String(Date.now())}.err`),
    });
...
export function createGeminiAdapter(): AiCliAdapter {
  return {
    ...
    async runTask(input: AiTaskInput): Promise<AiTaskResult> {
      ...
      const tmpDir = await mkdtemp(join(tmpdir(), "aiteam-gemini-"));

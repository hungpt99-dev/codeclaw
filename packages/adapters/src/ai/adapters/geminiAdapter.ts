...
async function checkGeminiAvailable(): Promise<boolean> {
  try {
    const result = await runShellCommand({
      ...
      cwd: process.cwd(),
      timeoutSeconds: 5,
      stdoutPath: join(tmpdir(), `codeclaw-gemini-check-${String(Date.now())}.out`),
      stderrPath: join(tmpdir(), `codeclaw-gemini-check-${String(Date.now())}.err`),
    });
...
export function createGeminiAdapter(): AiCliAdapter {
  return {
    name: "gemini",

    async isAvailable(): Promise<boolean> {
      return checkGeminiAvailable();
    },
    ...
    async runTask(input: AiTaskInput): Promise<AiTaskResult> {
      ...
      const tmpDir = await mkdtemp(join(tmpdir(), "codeclaw-gemini-"));

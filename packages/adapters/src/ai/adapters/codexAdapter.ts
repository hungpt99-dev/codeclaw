...
async function checkCodexAvailable(): Promise<boolean> {
  try {
    const result = await runShellCommand({
      ...
      cwd: process.cwd(),
      timeoutSeconds: 5,
      stdoutPath: join(tmpdir(), `codeclaw-codex-check-${String(Date.now())}.out`),
      stderrPath: join(tmpdir(), `codeclaw-codex-check-${String(Date.now())}.err`),
    });
...
export function createCodexAdapter(): AiCliAdapter {
  return {
    ...
    async runTask(input: AiTaskInput): Promise<AiTaskResult> {
      ...
      const tmpDir = await mkdtemp(join(tmpdir(), "codeclaw-codex-"));

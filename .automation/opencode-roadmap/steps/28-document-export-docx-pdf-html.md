...
The CLI already has an `codeclaw export` command planned (CLI Spec §31). This step implements it with support for multiple output formats:

```bash
codeclaw export <runId> --format docx    # Word document
codeclaw export <runId> --format pdf     # PDF
codeclaw export <runId> --format html    # Single HTML page
codeclaw export <runId> --format zip     # All formats bundled
codeclaw export <runId> --format markdown   # Original markdown (default)
```
...
- Wrap in a styled HTML document with:
  ...
  - Footer with page number and "Generated by CodeClaw"
...
### 6. Create CLI command: codeclaw export

Create `apps/cli/src/commands/export.ts`:

```bash
codeclaw export <runId> --format docx --output ./delivery/report.docx
codeclaw export <runId> --format pdf --output ./delivery/report.pdf
codeclaw export <runId> --format html --output ./delivery/report.html
codeclaw export <runId> --format zip --output ./delivery/run-export.zip
codeclaw export <runId> --format markdown --output ./delivery/docs/
```
...
- `codeclaw export --format pdf` should print:
  ```
...
- `codeclaw export <runId> --format docx` generates a Word document
- `codeclaw export <runId> --format pdf` generates a PDF (when playwright available)
- `codeclaw export <runId> --format html` generates a styled HTML file
- `codeclaw export <runId> --format zip` generates a ZIP archive
- `codeclaw export <runId> --format markdown` copies markdown files (default)

...
Features:
- `aiteam github status` — Check gh CLI availability and auth status
- `aiteam github test` — Test connection
- `aiteam github pr create --run <runId>` — Generate PR summary + optionally create PR (requires approval)
- `aiteam github pr view` — View PR details
- `aiteam github actions` — Read CI status
...
### 5. Create CLI command: aiteam github
...
aiteam github test
aiteam github pr create --run <runId> [--approve]
...
- Approval via `aiteam approve --gate EXTERNAL_UPDATE` or web UI
...
- `aiteam github status` shows gh CLI availability and auth status
- `aiteam github test` verifies connection
- `aiteam github pr create --run <runId>` generates PR summary and shows approval prompt
